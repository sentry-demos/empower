...
def get_products():
    ...
    try:
        ...
        query = text(
            "SELECT reviews.id, products.id AS productid, reviews.rating, reviews.customerid, "
            "reviews.description, reviews.created "
            "FROM reviews INNER JOIN products ON reviews.productId = products.id"
        )
        all_reviews = connection.execute(query).fetchall()

        for product in products:
            result = dict(product)
            result["reviews"] = [dict(r) for r in all_reviews if r[1] == product["id"]]
