...
function cleanup {
  ...
  stop.sh flask $FLASK_LOCAL_PORT 
}
...
gcloud compute ssh redis-relay --tunnel-through-iap --zone=us-central1-a -- -N -L $FLASK_LOCAL_REDISPORT:$REDIS_SERVER_IP:6379 &
...
gunicorn -b :$FLASK_LOCAL_PORT -w 2 --timeout 60 src.main:app
