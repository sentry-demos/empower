...
@contextmanager
def sentry_trace(headers, raw_body, body, latency):
    """
    ...
    with sentry_sdk.start_transaction(transaction):
        with sentry_sdk.start_span(op="queue.process", name="queue_consumer") as span:
            ...
            span.set_data("messaging.destination.name", body.get("TopicArn"))
            span.set_data("messaging.message.body.size", len(raw_body))
...
def lambda_handler(event, context):
    try:
        ...
        for record in records:
            ...
            with sentry_trace(headers, raw_body, body, latency) as span:
                ...
                for idx, quantity in enumerate(inventory_list):
                    if quantity < 0:
                        # print(f"throw syntax error { "headers" }")
                        raise ValueError(
