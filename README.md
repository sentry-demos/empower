...
- `?se=<yourname>&crash=true` - forces [a crash](https://github.com/sentry-demos/empower/blob/fce289530f72ce47fe2c7482cdbd9aa8bcc13b6e/react/src/utils/errors.js#L41) of one of predefined types, selected randomly. Requires `?se=` so that only demo/test traffic (SEs, TDA) can crash the app, never a real visitor.
- `?se=<yourname>&crash=true&errnum=3` - forces crash of specific type depending on `errnum` value
