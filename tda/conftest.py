...
#       -> webdriver.Remote.get(https://google.com/?se=prod-tda-...)
...
def se_base_tag():
    user = is_production() and 'prod' or get_system_user()
...
def se_prefix(request):
    ...
    if request.node.name.split('[')[0] != test_filepath.stem:
        raise Exception(f'TDA expects only 1 function in each test file with the same name as the \
...
def _local_browser(request, se):
    ...
    # TODO: pass `se` parameter value into ChromeWithExtraUrlParams as extra_params
    # for local browser and it will be set to: 
    #   prod-tda-mock-<github.run_id>_flask-<test_filepath>-local-chrome
