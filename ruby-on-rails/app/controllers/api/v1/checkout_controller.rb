...
class Api::V1::CheckoutController < ApplicationController
  def index
    ...
    params.each do |key, value|
      if key.to_s == "cart"
        ...
        validate_inventory = value.to_s == "true"
        Sentry.logger.debug("Inventory validation: %{validate}", validate: validate_inventory)
      end
    end
    ...
    if validate_inventory
      products_in_inventory.each_with_index { |inv_objs, i|
        if !enough_inventory?(cart_contents, inv_objs)
          begin
      ...
      Sentry.logger.info("Inventory validation bypassed per request parameter")
    end
  ...
  def enough_inventory?(cart_contents, inv_obj)
    # Check if the product in cart has sufficient inventory
    product_id = inv_obj["productid"].to_s
    
    if cart_contents.key?(product_id)
      requested_quantity = cart_contents[product_id].to_i
      available_quantity = inv_obj["quantity"].to_i
      
      if requested_quantity > available_quantity
        Sentry.logger.warn("Insufficient inventory for product %{product_id}: requested %{requested}, available %{available}", 
                          product_id: product_id, requested: requested_quantity, available: available_quantity)
        return false
      end
    end
