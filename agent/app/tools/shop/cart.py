...
async def add_to_cart(
    ...
    payload = {
        "cart": session.cart,
        "added": [{"product_id": pid, "qty": qty} for pid, qty in quantities.items()],
        "unknown_product_ids": unknown,
    }
    session.record_widget("add_to_cart", "cart", payload)
...
async def view_cart(context: RunContextWrapper[ChatSession]) -> str:
    ...
    payload = {"cart": session.cart, "promo": session.promo}
    session.record_widget("view_cart", "cart", payload)
    return json.dumps(payload)
...
async def start_checkout(context: RunContextWrapper[ChatSession]) -> str:
    ...
    if not session.cart["items"]:
        payload = {"error": "The cart is empty; add something first."}
        session.record_widget("start_checkout", "checkout", payload)
        return json.dumps(payload)
    ...
    payload = {"cart": session.cart, "form": session.form, "promo": session.promo}
    session.record_widget("start_checkout", "checkout", payload)
    return json.dumps(payload)
